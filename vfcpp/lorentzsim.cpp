...
std::vector<Eigen::MatrixXd> genCurve(int n_points, double vx0 = 0.999,
                                      double vxv = 0.0001, double vy0 = 0.999,
                                      double vyv = 0.0001) {
  ...
  for (int i = 0; i < n_points; i++) {
    ...
    double beta_x = vx0 + 0.5 * vxv * (1 + cos(s));
    ...
    // double beta_y = vy0 + 0.5 * vyv * (1 + sin(s));
    // double gamma_y = 1.0000 / sqrt(1.0 - pow(beta_y, 2.0));
    // Eigen::MatrixXd boost_y(4, 4);
    // boost_y << 1.0, 0, 0, 0, 
    //            0, gamma_y, 0, -gamma_y * beta_y, 
    //            0, 0, 1.0, 0, 
    //            0, -gamma_y * beta_y, 0, gamma_y;

    Eigen::MatrixXd boost = boost_x;

    curve.push_back(boost);
  }
  return curve;
}
...
std::vector<Eigen::MatrixXd> genCurveDerivative(int n_points,
  ...
  for (int i = 0; i < n_points; i++) {
    ...
    dboost_x << dgamma_x, 0.0, 0.0, -dgamma_x * beta_x - gamma_x * dbeta_x, 
                0, 0, 0, 0, 
                0, 0, 0, 0, 
                -dgamma_x * beta_x - gamma_x * dbeta_x, 0, 0, dgamma_x;

    // double beta_y = vy0 + 0.5 * vyv * (1 + sin(s));
    // double gamma_y = 1.0000 / sqrt(1.0 - pow(beta_y, 2.0));
    // double dbeta_y = 0.5 * vyv * cos(s);
    // double dgamma_y = (beta_y / pow(1.0 - pow(beta_y, 2), 1.5)) * dbeta_y;
    // Eigen::MatrixXd boost_y(4, 4);
    // boost_y << 1.0, 0, 0, 0, 
    //            0, gamma_y, 0, -gamma_y * beta_y, 
    //            0, 0, 1.0, 0, 
    //            0, -gamma_y * beta_y, 0, gamma_y;
    // Eigen::MatrixXd dboost_y(4, 4);
    // dboost_y << 0.0, 0, 0, 0, 
    //             0, dgamma_y, 0, -dgamma_y * beta_y - dbeta_y * gamma_y, 
    //             0, 0, 0, 0,
    //             0.0, -dgamma_y * beta_y - dbeta_y * gamma_y, 0.0, dgamma_y;

    Eigen::MatrixXd dboost = dboost_x;

    dcurve.push_back(dboost);
  }
  return dcurve;
}

void writeToCSV(
    const std::vector<std::tuple<Eigen::MatrixXd, Eigen::VectorXd,
                                 Eigen::VectorXd, double>>& iterationResults,
...
void write_curve2csv(const std::vector<Eigen::MatrixXd>& curve,
                     const std::string& filename) {
  std::ofstream csvFile;
  csvFile.open(filename);
...
void test_VectorField() {
  ...
  std::vector<Eigen::MatrixXd> dcurve =
      genCurveDerivative(npoints, vel_x, vel_x_var, vel_y, vel_y_var);
  ...
  // std::cout << "Euclidean distance between state0 and curve[0]: " << dist <<
  // std::endl; double min_dist = vf.ECdistance(state0); std::cout << "Minimum
  // distance between state0 and curve: " << min_dist << std::endl;
  ...
  for (int i = 0; i < n_steps; i++) {
    ...
    //     std::cout << "State matrix: " << std::endl << state.matrix() <<
    //     std::endl;
    // }
    // std::cout << "Call eval" << std::endl;
    Eigen::VectorXd xi =
        vf.eval(state, true, 1.0, gain_N, gain_T1, 1.0, gain_T);
    ...
    // print determinants:
    // std::cout << "Determinant of state: " << state.matrix().determinant() <<
    // std::endl; Eigen::Matrix3d R_state = state.matrix().block<3, 3>(0, 0);
    // Eigen::Matrix3d R_debug = debug_mat.block<3, 3>(0, 0);
    // Checks R*R^T
    // std::cout << "R*R^T: " << std::endl << (R_state *
    // R_state.transpose().eval() - Eigen::Matrix3d::Identity()).norm() <<
    // std::endl; std::cout << "R_debug*R_debug^T: " << std::endl << (R_debug *
    // R_debug.transpose().eval() - Eigen::Matrix3d::Identity()).norm() <<
    // std::endl;
  ...
  writeToCSV(vf.iterationResults,
             "/home/fbartelt/Documents/Projetos/vector-field/vfcpp/logs/"
             "LORENTZ_vf_data.csv");
  write_curve2csv(curve,
                  "/home/fbartelt/Documents/Projetos/vector-field/vfcpp/logs/"
                  "LORENTZ_curve_data.csv");
  write_curve2csv(updated_states,
                  "/home/fbartelt/Documents/Projetos/vector-field/vfcpp/logs/"
                  "LORENTZ_iteration_data.csv");

  // Write to csv a vector<double>:
  std::ofstream csvFile;
  csvFile.open(
      "/home/fbartelt/Documents/Projetos/vector-field/vfcpp/logs/"
      "LORENTZ_closeidx.csv");
